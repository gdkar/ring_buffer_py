...
test_init(){
    ...
    mu_assert("ring_buffer created and freed",1 == 1);
    return 0;
}
...
test_read(){
    ...
    mu_assert("ring_buffer read correctly returned the char[] array",is_the_same == 0);
    ring_buffer_free (buffer);
}
...
test_eof(){
    ...
    mu_assert("ring_buffer_eof should return a non-zero integer",ring_buffer_eof (buffer));
    ring_buffer_free (buffer);
}
...
test_peek(){
    ...
    mu_assert("ring_buffer_peek should assign 4 chars to the char array peek_data",is_the_same);
    ring_buffer_free (buffer);
}
